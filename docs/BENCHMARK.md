...
**対象ハードウェア（v1）:** `icosahedron-15` / **15panels** env（225 LED、5 データ線、GPIO 16/17/18/19/21）
...
1. `tools/layout-compile.ts` でレイアウトをコンパイル。
2. ファームをフラッシュ（`firmware/esp32`）。
...
- ボトルネックは帯域より **ESP の LED 出力時間（NeoPixelBus I2S0）+ Wi-Fi スタック遅延** になりやすい

### ESP32（`60panels`）— 2026-07-01

| 項目 | 記録 |
|------|------|
| レイアウト | `geodesic-2v-60`（1260 LED、10 線、105/147 混在） |
...
| 備考 | `GLOWBE_PLAYOUT_LAG_FRAMES=0`（60panels プロファイル）。送信 fps は実効 **~165 fps** 以下に合わせる |

送信 fps は ESP の実効上限（60panels でおおよそ **150–170 fps**）以下に合わせること。

## 合格記録（暫定）

### ESP32（`15panels`）— 2026-06-14

| 項目 | 記録 |
|------|------|
| レイアウト | `icosahedron-15`（225 LED、5 線） |
| LED 出力 | **NeoPixelBus I2S0 並列**。再フラッシュして再計測すること。 |
