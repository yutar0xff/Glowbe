...
Loop（クリップ再生中）・Interactive・Mate の各 Studio タブに組み込み済み。ランタイムの WebSocket `previewSubscribe` とバイナリ LED フレーム（約 30fps、**トーン適用前**の RGB）で 2D / 3D マップを更新する。仕様は [`protocol/control-api.md`](../protocol/control-api.md) を参照。
